...
- [x] Vue.config.preserveWhitespace <sup>new - turn it off to ignore whitespaces between elements in templates. Improves performance.</sup>
- [x] Vue.config.silent
...
#### How to Deal with the Deprecation of Filters?

Filters have proven to be little more useful than just plain methods - they introduce an additional special syntax that is not JavaScript, and two-way filters can be easily misused, leading to complexity.

For simple data formatting needs, you can either just define a method instead of a filter, or use computed properties instead.
