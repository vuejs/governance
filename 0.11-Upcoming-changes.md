...
- [Instantiation process](#instantiation-process-__breaking__)
- [New Scope Inheritance Model](#new-scope-inheritance-model-__breaking__)
- [Instance Option changes](#instance-option-changes-__breaking__)
- [Instance methods change](#instance-methods-change)
- [Computed Properties API Change](#computed-properties-api-change-__breaking__)
- [Directive API change](#directive-api-change)
- [Interpolation change](#interpolation-change)
- [Config API change](#config-api-change-__breaking__)
- [Transition API Change](#transition-api-change-__breaking__)
- [Two Way filters](#two-way-filters-__new__)
- [Block logic control](#block-logic-control-__new__)
- [Events API](#events-api-__new__)

## Instantiation process __BREAKING__
...
## New Scope Inheritance Model __BREAKING__
...
## Instance Option changes __BREAKING__
...
- __BREAKING__ `$unwatch` has been removed. `$watch` now also returns an unregister function:
...
- #### New methods
  ...
  - `vm.$add` and `vm.$delete`: explicitly add/remove properties from the ViewModel. Observed objects are augmented with these two methods too. Use these only when needed - the best practice is to define all your data fields at instantiation, even with a `null` value.

  - `vm.$eval`: evaluate an expression that can also include filters.

  ``` js
  var value = vm.$eval('msg | uppercase')
  ```

  - `vm.$interpolate`: evalutate a piece of template string.

  ``` js
  var markup = vm.$interpolate('<p>{{msg | uppercase}}</p>')
  ```

  - `vm.$log`: log the current instance data as a plain object, which is more console-inspectable than a bunch of getter/setters. Also accepts an optional key.
...
## Computed Properties API Change __BREAKING__
...
- __BREAKING__ Text bindings will no longer automatically stringify objects. Use the new `json` filter which gives more flexibility in formatting.

- #### One time interpolations __NEW__
...
## Config API change __BREAKING__
...
## Transition API Change __BREAKING__
